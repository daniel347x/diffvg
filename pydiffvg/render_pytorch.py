...
class RenderFunction(torch.autograd.Function):
    ...
    def serialize_scene(
        ...
        shapes,
        shape_groups,
        filter=pydiffvg.PixelFilter(
            type=diffvg.FilterType.box, radius=torch.tensor(0.5)
        ),
        ...
        for shape in shapes:
            ...
            if isinstance(shape, pydiffvg.Circle):
                ...
                if (
                    len(shape.stroke_width.shape) > 0
                    and shape.stroke_width.shape[0] > 1
                ):
                ...
                if shape.is_closed:
                    ...
                    args.append(
                        torch.zeros(shape.points.shape[0] - 1, dtype=torch.int32)
                    )
                args.append(shape.points.cpu())
                args.append(None)
                args.append(shape.is_closed)
        ...
        *args,
        ...
        for shape_id in range(num_shapes):
            ...
            if shape_type == diffvg.ShapeType.circle:
                ...
                current_index += 1
                shape = diffvg.Path(
                    diffvg.int_ptr(num_control_points.data_ptr()),
                    diffvg.float_ptr(points.data_ptr()),
                    diffvg.float_ptr(
                        thickness.data_ptr() if thickness is not None else 0
                    ),
                ...
                current_index += 1
                shape = diffvg.Rect(
                    diffvg.Vector2f(p_min[0], p_min[1]),
            ...
            current_index += 1
            shapes.append(
                diffvg.Shape(shape_type, shape.get_ptr(), stroke_width.item())
            )
        ...
        scene = diffvg.Scene(
            ...
            pydiffvg.get_device().index
            if pydiffvg.get_device().index is not None
            else -1,
        )
        ...
        if output_type == OutputType.color:
            ...
            if eval_positions.shape[0] == 0:
                rendered_image = torch.zeros(
                    height, width, 1, device=pydiffvg.get_device()
                )
            else:
                rendered_image = torch.zeros(
                    eval_positions.shape[0], 1, device=pydiffvg.get_device()
                )
        ...
        diffvg.render(
            scene,
            diffvg.float_ptr(
                background_image.data_ptr() if background_image is not None else 0
            ),
            diffvg.float_ptr(
                rendered_image.data_ptr() if output_type == OutputType.color else 0
            ),
            diffvg.float_ptr(
                rendered_image.data_ptr() if output_type == OutputType.sdf else 0
            ),
            width,
            height,
            num_samples_x,
    ...
    def render_grad(
        ...
        *args,
        ...
        translation_grad_image = torch.zeros(
            height, width, 2, device=pydiffvg.get_device()
        )
        start = time.time()
        diffvg.render(
            scene,
            ...
            diffvg.float_ptr(0),  # d_background_image
            diffvg.float_ptr(
                grad_img.data_ptr() if output_type == OutputType.color else 0
            ),
            diffvg.float_ptr(
                grad_img.data_ptr() if output_type == OutputType.sdf else 0
            ),
    ...
    def backward(ctx, grad_img):
        ...
        diffvg.render(
            ...
            seed,
            diffvg.float_ptr(
                d_background_image.data_ptr() if background_image is not None else 0
            ),
            diffvg.float_ptr(
                grad_img.data_ptr() if output_type == OutputType.color else 0
            ),
        ...
        for shape_id in range(scene.num_shapes):
            ...
            if d_shape.type == diffvg.ShapeType.circle:
                ...
                if d_path.has_thickness():
                    ...
                    d_path.copy_to(
                        diffvg.float_ptr(points.data_ptr()),
                        diffvg.float_ptr(thickness.data_ptr()),
                    )
                else:
                    d_path.copy_to(
                        diffvg.float_ptr(points.data_ptr()), diffvg.float_ptr(0)
                    )
        ...
        for group_id in range(scene.num_shape_groups):
            ...
            if d_shape_group.has_stroke_color():
                if d_shape_group.stroke_color_type == diffvg.ColorType.constant:
                    ...
                    d_shape_group.stroke_color_type == diffvg.ColorType.linear_gradient
                ):
